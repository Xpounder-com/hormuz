    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        with self.lock:
            ...
            type(self).provider_authorization_seen |= (
                self.headers.get("Authorization") == "Bearer replace-with-openai-provider-key"
            )
            type(self).capped_output_seen |= body.get("max_output_tokens") == 64
            type(self).routed_model_seen |= body.get("model") == "gpt-compose-proof"
        self._send(
            {
                "id": "resp_compose_proof",
                "object": "response",
                "status": "completed",
            ...
            request_id="req_compose_proof",
        )

    def _send(self, value: dict[str, object], *, status: int = 200, request_id: str | None = None) -> None:
