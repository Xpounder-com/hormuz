...
export default function PrivacyPage() {
  return <PageFrame active="privacy">
    ...
    <section className="section prose-section">
      ...
      <p>Fonts and the recorded demo are served with the site; no third-party video player is embedded. Optional X Ads measurement stays off until you allow it using the privacy choices on this website.</p>
      ...
      <p>Selected application links retain only bounded utm_source, utm_medium, and utm_campaign parameters from the current page URL. The application includes them with your inquiry only if you select the unchecked campaign-source checkbox. This choice is separate from optional X Ads measurement and does not change the initial URL request already received by GitHub Pages. Do not put personal information or secrets in campaign URLs.</p>
      <h2 id="advertising">Optional X Ads measurement</h2>
      <p>If you choose Allow measurement, the X Pixel measures site visits and applications acknowledged by Formspree. X may use browser and network information, cookies, and an X ad click identifier to associate activity with ads and improve advertising. An application event contains no name, email address, organization, workflow text, payment amount, or other form entry. We enable X’s page-location suppression and do not create website activity audiences for this event.</p>
