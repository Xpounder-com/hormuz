...
struct CompanionLayout {
    ...
    var percentFontSize: CGFloat { p(27) / 0.714 }
    var cardTitleFontSize: CGFloat { p(26) / 0.714 }
    var cardBodyFontSize: CGFloat { p(18) / 0.714 }

    var percentLineHeight: CGFloat {
        lineHeight(NSFont.systemFont(ofSize: percentFontSize, weight: .semibold))
    }
    ...
    var cellExtent: CGFloat { ringDiameter + ringLabelGap + percentLineHeight }
    ...
    func bodyHeight(metricCount: Int) -> CGFloat {
        CGFloat(CompanionGeometry.bodyHeight(
            ...
            percentLineHeight: Double(percentLineHeight),
    ...
    // The Hormuz footer makes the reference card taller while preserving its width and rhythm.
    func cardHeight(for reading: CompanionMetricReading?) -> CGFloat {
        guard let reading else { return p(474) }
        if reading.blocks.isEmpty { return p(390) }
        if reading.message != nil { return p(540) }
        return p(474)
    }
