...
_POSTGRES_EXPECTED_ACL_BOUNDARY_BY_VERSION = {
    ...
    # Twice measured from independent clean managed-role schema-18 bootstraps.
    18: (
        205,
    ...
    # Twice measured from independent clean managed-role schema-24 bootstraps
    # after adding the four append-only recommendation snapshot, lifecycle,
    # read-audit, and cursor tables, adding one tenant-scoped policy activation
    # receipt function, and revoking PostgreSQL's 20 default PUBLIC grants.
    24: (
        249,
        ...
        with connection.transaction():
            with connection.cursor() as cursor:
                ...
                if states:
                    ...
                    if max(states) >= 24:
                        from ._recommendation_schema import verify_postgres_recommendations
                        verify_postgres_recommendations(cursor, schema, PostgresStorageError)
        ...
        with connection.cursor() as cursor:
            ...
            if (
                ...
                if max(states) >= 15:
                    from ._finance_attempt_schema import verify_postgres_finance_attempt
                    verify_postgres_finance_attempt(cursor, schema, PostgresStorageError)
                # Schema 16 installs the collection tables for owner-side
                # qualification; schema 17 is the first version that grants
                # the runtime role access to verify them.
                if max(states) >= 17:
                    from ._finance_collection_schema import verify_postgres_finance_collection
                    verify_postgres_finance_collection(cursor, schema, PostgresStorageError)
                if max(states) >= 18:
...
def _migration_sql(
    ...
    filenames = {
        ...
        24: "0024_policy_recommendations.sql",
    }
