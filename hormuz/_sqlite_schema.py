...
def verify_applied_sqlite_schema_shape(
    ...
    *,
    ...
    if version >= 8:
        from ._finance_schema import verify_sqlite_finance
        verify_sqlite_finance(connection, error_factory)
    if version >= 9:
        from ._budget_schema import verify_sqlite_budget
        verify_sqlite_budget(connection, error_factory)
    if version >= 10:
        from ._provider_reliability_schema import verify_sqlite_provider_reliability
        verify_sqlite_provider_reliability(connection, error_factory)
    if version >= 11:
        from ._finance_attempt_schema import verify_sqlite_finance_attempt
        verify_sqlite_finance_attempt(connection, error_factory)
    if version >= 12:
        from ._finance_collection_schema import verify_sqlite_finance_collection
        verify_sqlite_finance_collection(connection, error_factory)
    if version >= 13:
        from ._finance_account_binding_schema import verify_sqlite_finance_account_binding
    ...
    if version >= 16:
        from ._association_schema import verify_sqlite_association
        verify_sqlite_association(connection, error_factory)
    if version >= 17:
        from ._scorecard_schema import verify_sqlite_scorecards
        verify_sqlite_scorecards(connection, error_factory)
    if version >= 18:
        from ._role_view_schema import verify_sqlite_role_views
        verify_sqlite_role_views(connection, error_factory)
    if version >= 19:
        from ._recommendation_schema import verify_sqlite_recommendations
        verify_sqlite_recommendations(connection, error_factory)
...
def apply_sqlite_migration(
    ...
    *,
    ...
    if version == 7:
        ...
        return
    if version == 8:
        from ._finance_schema import sqlite_statements
        _execute_statements(connection, sqlite_statements())
        return
    if version == 9:
        from ._budget_schema import sqlite_statements
        _execute_statements(connection, sqlite_statements())
        return
    if version == 10:
        from ._provider_reliability_schema import sqlite_statements
        _execute_statements(connection, sqlite_statements())
        return
    if version == 11:
        from ._finance_attempt_schema import apply_sqlite_finance_attempt_migration
        apply_sqlite_finance_attempt_migration(connection)
        return
    if version == 12:
        from ._finance_collection_schema import apply_sqlite_finance_collection_migration
        apply_sqlite_finance_collection_migration(connection)
        return
    if version == 13:
        from ._finance_account_binding_schema import apply_sqlite_finance_account_binding_migration
        apply_sqlite_finance_account_binding_migration(connection)
        return
    if version == 14:
    ...
    if version == 15:
        ...
        return
    if version == 16:
        from ._association_schema import apply_sqlite_association_migration
        apply_sqlite_association_migration(connection)
        return
    if version == 17:
        from ._scorecard_schema import apply_sqlite_scorecard_migration
        apply_sqlite_scorecard_migration(connection)
        return
    if version == 18:
        from ._role_view_schema import apply_sqlite_role_view_migration
        apply_sqlite_role_view_migration(connection)
        return
    if version == 19:
        from ._recommendation_schema import apply_sqlite_recommendation_migration
        apply_sqlite_recommendation_migration(connection)
        return
