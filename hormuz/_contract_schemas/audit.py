    ...
    *,
    ...
    if source_schema_id == "hormuz.custody-control-event" and source_schema_version == 1:
        ...
        from ..association_evidence import (
            ASSOCIATION_SOURCE_SCHEMA_IDS,
            association_source_identity,
        )

        if source_schema_id in ASSOCIATION_SOURCE_SCHEMA_IDS:
            try:
                expected_source_id = association_source_identity(source_schema_id, event)
            except ValueError as error:
                raise ContractValidationError(
                    "association audit source is invalid"
                ) from error
        else:
