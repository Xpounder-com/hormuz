#!/usr/bin/env bash
# Run the Ceph/OpenBao key-version rotation recovery gate from the pinned
# linux/amd64 custody runner. This is an opt-in disposable-lab command only.
...
readonly ENVIRONMENT_FILE="${HORMUZ_CEPH_CUSTODY_ROTATION_RECOVERY_ENV_FILE:-}"
...
if [[ "$#" -ne 2 || "$1" != "--evidence-out" ]]; then
  failure 'usage: run_ceph_rgw_custody_rotation_recovery_container.sh --evidence-out /secure/path/evidence.json'
fi
if [[ -z "${ENVIRONMENT_FILE}" || ! -f "${ENVIRONMENT_FILE}" || ! -r "${ENVIRONMENT_FILE}" ]]; then
  failure 'recovery_environment_file_required'
fi
...
docker run --rm \
  ...
  --entrypoint python \
  "${runner_image_digest}" \
  /opt/hormuz/tools/verify_ceph_rgw_custody_rotation_recovery.py \
