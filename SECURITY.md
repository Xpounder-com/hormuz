...
- Configurable egress controls inspect high-confidence credential formats, every configured Hormuz/provider credential, valid hyphenated US SSNs, Luhn-valid card candidates, low-confidence email syntax, and bounded organization dictionaries before upstream serialization. They also decode bounded UTF-8 base64/base64url strings and textual data URIs up to three layers before applying the same rules, then re-encode only transformed values.
...
- Structured DLP remains deterministic and does not understand image/file contents. Recognized provider media blocks are denied rather than decoded. Supported encoded text is limited to single-string standard or URL-safe base64 that decodes to bounded, predominantly printable UTF-8, including textual data URIs; whitespace-wrapped encodings, compression, archives, binary payloads, other encodings, caller-controlled provider headers and JSON keys, source paths, and semantically sensitive company information remain unclassified. Email matching remains detect-only; organization evaluation, approval notifications/queue UX, and HA approval persistence are open gates.
