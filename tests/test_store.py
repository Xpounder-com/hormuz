        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            def verify_then_apply(connection: sqlite3.Connection, version: int) -> None:
                ...
                if version == 3:
                    ...
                    self.assertIn("portfolio_work_scope_versions", names)
                    self.assertFalse(any(name.startswith("portfolio_attribution_") for name in names))
                elif version == 7:
                    ...
                    self.assertIn("portfolio_finance_rate_cards", names)
                    self.assertFalse(any(name.startswith("portfolio_work_budget_") for name in names))
                elif version == 10:
                    self.assertIn("portfolio_work_budget_plan_versions", names)
                    self.assertFalse(any(name.startswith("gateway_provider_") for name in names))
                elif version == 11:
                    ...
                    self.assertIn("portfolio_finance_source_binding_versions", names)
                    self.assertNotIn("portfolio_finance_account_binding_versions", names)
                elif version == 14:
                    self.assertIn("portfolio_finance_account_binding_versions", names)
                    self.assertNotIn("portfolio_linear_source_binding_versions", names)
                elif version == 15:
                    self.assertIn("portfolio_linear_source_binding_versions", names)
                    self.assertNotIn("gateway_linear_snapshot_receipts", names)
                    self.assertNotIn("portfolio_linear_snapshot_context_events", names)
                elif version == 16:
                    ...
                    self.assertIn("portfolio_model_scorecard_snapshots", names)
                    self.assertIn("portfolio_scorecard_audit_events", names)
                    self.assertNotIn("portfolio_role_view_audit_events", names)
                    self.assertNotIn("portfolio_role_view_cursors", names)
                elif version == 19:
                    self.assertIn("portfolio_role_view_audit_events", names)
                    self.assertIn("portfolio_role_view_cursors", names)
                    self.assertNotIn("portfolio_policy_recommendation_snapshots", names)
                    self.assertNotIn("portfolio_policy_recommendation_events", names)
                else:
            ...
            self.assertEqual(
                migrations,
                [(version, "applied") for version in range(1, 20)],
            )
