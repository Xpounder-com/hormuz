...
class PostgresFinanceTransitionTests(PostgresTestCase):
    def migrate(self):
    ...
    def setUp(self):
        ...
        self.assertEqual(len(self.before["rows"]), 51)
    ...
    def restore(self, backup):
        database = "finance_restore_" + uuid4().hex[:12]
    ...
    def test_postgres_finance_real_migration_and_missing_following_migration(self):
        ...
        # Schemas through 23 now exist; schema 24 remains intentionally absent.
    ...
    def test_postgres_finance_partial_state_refuses_before_repair(self):
        with self.psycopg.connect(self.owner_dsn) as connection:
            connection.execute(self.sql.SQL("INSERT INTO {}.hormuz_schema_migrations (version, state) VALUES (12, 'applying')").format(self.sql.Identifier(self.schema)))
    ...
    def test_postgres_finance_actual_old_binaries_refuse_next_and_partial_schema(self):
        ...
        self.assertEqual(self.snapshot(), before)
        with self.psycopg.connect(self.owner_dsn) as connection:
            connection.execute(self.sql.SQL("UPDATE {}.hormuz_schema_migrations SET state='applying' WHERE version=12").format(self.sql.Identifier(self.schema)))
