"""Use the actual accepted outcome binary and its digest-bound fixtures.

The archive is an intermediate Git checkpoint, not a published release. No
fixture source is taken from the evolving finance checkout inside the driver.
"""
...
SOURCE_COMMIT = "aa648edf64df9f4a0c426ad73a95852f11561099"
ARCHIVE_SHA256 = "fbbe1178607a38c5f390b96c646f5a93414523b37f553abb7d830d42f30ff056"


def verify_installed_runtime(source_tar, package_root):
    """Do not trust install metadata if source/data files drift afterwards."""
    prefix = "hormuz-outcome-baseline/hormuz/"
    try:
        expected = {}
        for member in source_tar.getmembers():
            ...
            if (not member.isfile() or member.size > 2 * 1024 * 1024
                ...
                raise RuntimeError("outcome_predecessor_runtime_mismatch")
        ...
        raise RuntimeError("outcome_predecessor_runtime_mismatch") from None


def outcome_predecessor_call(request):
    executable = os.environ["HORMUZ_TEST_OUTCOME_PYTHON"]
    ...
    if result.returncode:
        raise AssertionError("outcome_predecessor_driver_failed")
...
def _driver():
    ...
    if (distribution.version != "1.0.0" or UsageStore.schema_version != 7 or POSTGRES_SCHEMA_VERSION != 11
        ...
        raise RuntimeError("outcome_predecessor_binding_invalid")
    ...
    with tarfile.open(fileobj=io.BytesIO(archive_bytes), mode="r:") as source_tar:
        ...
        for name in ("_registry_transition_fixture", "_portfolio_fixture", "_attribution_fixture", "test_outcome_contract", "_outcome_fixture"):
            member = source_tar.getmember(f"hormuz-outcome-baseline/tests/{name}.py")
            if not member.isfile() or member.size > 128 * 1024:
                raise RuntimeError("outcome_predecessor_fixture_invalid")
    ...
    try:
        ...
        if request["backend"] == "sqlite":
            ...
            raise RuntimeError("outcome_predecessor_backend_invalid")
        ...
        if request["mode"] == "seed":
            ...
            raise RuntimeError("outcome_predecessor_mode_invalid")
