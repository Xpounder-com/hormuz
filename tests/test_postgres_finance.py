...
class PostgresFinanceTests(FinanceAssertions, PostgresTestCase):
    ...
    def test_postgres_finance_real_schema_and_checked_in_migration(self):
        self.assertEqual(POSTGRES_SCHEMA_VERSION, 18)
        self.assertEqual(len(self.legacy_rows()) + len(TABLE_DDL), 71)
    ...
    def test_postgres_finance_direct_repository_refuses_partial_missing_and_newer_ledger(self):
        ...
        changes = (
            (f"UPDATE {{}}.hormuz_schema_migrations SET state='applying' WHERE version={current_version}",
             f"UPDATE {{}}.hormuz_schema_migrations SET state='applied' WHERE version={current_version}"),
            (f"UPDATE {{}}.hormuz_schema_migrations SET version={current_version + 1} WHERE version={current_version}",
             f"UPDATE {{}}.hormuz_schema_migrations SET version={current_version} WHERE version={current_version + 1}"),
        )
        ...
        with self.psycopg.connect(self.owner_dsn) as connection:
            saved = connection.execute(self.sql.SQL(
                f"DELETE FROM {{}}.hormuz_schema_migrations WHERE version={current_version} "
                "RETURNING version,state,applied_at"
            ).format(self.sql.Identifier(self.schema))).fetchone()
        try:
            self.error("unavailable", lambda: self.register(version=2))
            self.error("unavailable", self.get)
            self.assertEqual(self.finance_rows(), before)
