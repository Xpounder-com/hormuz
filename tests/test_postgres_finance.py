...
class PostgresFinanceTests(FinanceAssertions, PostgresTestCase):
    ...
    def test_postgres_finance_real_schema_and_checked_in_migration(self):
        self.assertEqual(POSTGRES_SCHEMA_VERSION, 12)
        self.assertEqual(len(self.legacy_rows()) + len(TABLE_DDL), 53)
    ...
    def test_postgres_finance_direct_repository_refuses_partial_missing_and_newer_ledger(self):
        ...
        before = self.finance_rows()
        changes = (
            ("UPDATE {}.hormuz_schema_migrations SET state='applying' WHERE version=12",
             "UPDATE {}.hormuz_schema_migrations SET state='applied' WHERE version=12"),
            ("UPDATE {}.hormuz_schema_migrations SET version=13 WHERE version=12",
             "UPDATE {}.hormuz_schema_migrations SET version=12 WHERE version=13"),
        )
        ...
        with self.psycopg.connect(self.owner_dsn) as connection:
            saved = connection.execute(self.sql.SQL("DELETE FROM {}.hormuz_schema_migrations WHERE version=12 RETURNING version,state,applied_at").format(self.sql.Identifier(self.schema))).fetchone()
        try:
            self.error("unavailable", lambda: self.register(version=2))
            self.error("unavailable", self.get)
            self.assertEqual(self.finance_rows(), before)
