...
class RepositoryGovernanceTests(unittest.TestCase):
    ...
    def test_windows_preview_checks_and_provenance_cannot_be_removed(self) -> None:
        mutations = (
            ...
            ("lifecycle_sha256 = (Get-FileHash target/release/windows-lifecycle.json -Algorithm SHA256).Hash.ToLowerInvariant()", 'lifecycle_sha256 = "unverified"'),
    ...
    def test_macos_distribution_must_require_exact_default_branch_source(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            self._copy_contract(root)
            workflow = root / ".github/workflows/macos-distribution.yml"
