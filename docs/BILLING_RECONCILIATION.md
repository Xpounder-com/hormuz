...
Each page is limited to 16 MiB, the complete report to 32 MiB and 100 pages, and duplicate JSON object members and non-standard numeric constants fail closed. Authenticated date windows are limited to 366 days. Billing evidence uses the configured SQLite or opt-in PostgreSQL accounting store. Scheduled polling, secure hosted credential custody, billing-operator RBAC, encrypted/KMS-backed storage, invoice/credit reconciliation, retention, HA, and immutable external audit remain enterprise release work.
