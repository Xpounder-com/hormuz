...
The decoder does not treat provider image/file blocks as text. Those blocks remain governed by `opaque_media`. It also does not unwrap whitespace-formatted base64, percent/hex encodings, unknown binary formats, encrypted/obfuscated containers without a recognized signature or declared media type, or provider file/URL references. These are explicit residual boundaries rather than claims of inspection.
