...
The broader boundary remains governed by [accepted ADR 0004](decisions/0004-structured-dlp-and-approval-boundary.md). Source classification, application-specific query parsing beyond the bounded decoded views, semantic detection, unknown encoding/container coverage and archive-content inspection, actual organization-representative evaluation and threshold approval, multi-node approval persistence/notification, content-cache invalidation, and independent review are still open. Issue #10 remains open until those paths and the complete compatibility, failure, migration, and privacy gates pass.
