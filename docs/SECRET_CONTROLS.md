...
The encoded-string decoder does not treat provider image/file blocks as text. Those blocks remain governed by `opaque_media`. For ordinary JSON strings it also does not unwrap non-ASCII whitespace, percent/hex or other transfer encodings, unknown binary formats, encrypted/obfuscated containers without a recognized signature or declared media type, or provider file/URL references. Provider URL queries have their separate bounded percent-decoding contract below. These are explicit residual boundaries rather than claims of inspection.
