...
Ordinary JSON strings also receive bounded recursive inspection of printable UTF-8 hidden by `%HH` percent escapes or hexadecimal text, including an optional `0x` prefix. Percent decoding does not reinterpret `+` as space. Safe and detect-only strings remain byte-for-byte unchanged. Because rewriting those forms can alter application-specific syntax, a credential or DLP rule configured for redaction fails closed for the hidden occurrence instead of claiming a safe transformation. A literal outer occurrence is still transformed normally, and Hormuz subtracts that visible finding from the decoded view so one logical value is not double-counted. A weaker visible detect-only finding cannot mask a stronger encoded credential in the same string.

It decodes at most 1 MiB per value, accepts at most 2,796,208 characters in a whitespace-wrapped payload, and processes at most three nested encoding layers. The nesting check applies before text or archive classification, so a recognized archive hidden behind a fourth encoded layer fails closed instead of bypassing the depth limit. The same credential and structured-DLP rules apply to decoded text. Detect, deny, and approval-required outcomes retain the original encoded value in memory while following their ordinary forwarding or fail-closed behavior. For base64, a direct exact-secret or DLP match on the outer value takes precedence, preserving protection for secrets that are themselves base64-shaped; percent/hex views instead combine only newly revealed findings with direct findings.
...
The encoded-string decoder does not treat provider image/file blocks as text. Those blocks remain governed by `opaque_media`. For ordinary JSON strings it does not unwrap non-ASCII base64 whitespace, non-UTF-8 percent/hex payloads, transfer encodings other than the named base64, percent, and hexadecimal forms, unknown binary formats, encrypted/obfuscated containers without a recognized signature or declared media type, or provider file/URL references. Provider URL queries have their separate form-plus-percent decoding contract below. These are explicit residual boundaries rather than claims of inspection.
...
Hormuz forwards only a small provider-compatible subset of caller headers. Both protocols may receive caller-supplied `Accept` and `User-Agent`; OpenAI may additionally receive `OpenAI-Beta`; Anthropic may additionally receive `Anthropic-Version` and `Anthropic-Beta`. Every forwarded value must be no more than 1,024 bytes of visible ASCII plus horizontal tab. Folded, control-bearing, non-ASCII, or overlong metadata receives a fixed `400 invalid_request` before DLP, policy accounting, provider work, or usage persistence. Hormuz supplies `Content-Type` and the provider authorization credential itself, so caller `Authorization` and `X-Api-Key` values are never forwarded.
...
Hormuz extracts the exact raw query that it will append to the provider URL and keeps that raw query unchanged for forwarding. It first form-decodes the complete query as strict UTF-8, covering parameter names and values, `%HH` encoding, and `+`-encoded spaces. It then percent-decodes only `%HH` sequences in each resulting view; a literal `+`, including one produced from `%2B`, is not reinterpreted as a space on those later passes. Hormuz inspects every distinct view through a maximum of three decoding passes. A fourth changing layer or invalid UTF-8 at any layer fails closed before provider work with a content-free validation error.

All views describe one logical raw query. The same finding visible in more than one view therefore contributes the highest count observed in a single view rather than being counted repeatedly. This preserves intermediate exact-value coverage without inflating DLP evidence. Every view also passes through the supported encoded-text detector.
...
## Organization-labeled detector evaluation

`hormuz dlp evaluate` runs one enabled organization detector against a strict local JSONL corpus using the same provider-aware inspection kernel as the request path. It makes no provider call, applies no redaction/denial/approval side effect, and writes no usage, security, approval, or context record. Its versioned report contains only aggregate confusion counts and metrics plus safe detector, policy, corpus-version, provider, and model metadata.

The corpus itself is content-bearing and must remain in the organization's controlled boundary. The report deliberately omits payloads, matches, examples, case identifiers, dictionary values, environment-variable names, filenames, and corpus hashes. The caller-supplied `corpus_id` is a version label rather than a claim that Hormuz can attest to corpus custody. See [DLP_EVALUATION.md](DLP_EVALUATION.md) for the exact schema, resource limits, command, and manual promotion workflow.
...
This is a bounded deterministic DLP subset, not a complete data-loss-prevention system. Detector version `hormuz-deterministic-v2` inspects JSON string values and keys, up to three decoded provider-query views, the supported base64, percent, and hexadecimal text forms and ASCII MIME whitespace above, the exact forwarded provider-header allowlist, and known provider media shapes. It denies recognized opaque media and encoded archive/container formats but does not inspect their contents, identify every binary or obfuscated container, model application-specific parsing beyond those bounded views, unwrap unsupported encodings, classify source paths, infer proprietary meaning, or reliably detect transformed and obfuscated values. A custom exact value protects only that exact case-sensitive textual representation and its occurrence in supported decoded text. The SSN detector intentionally supports only the high-confidence hyphenated form; the bundled email detector has not passed an actual customer organization-specific false-positive/false-negative evaluation and therefore remains detect-only. Hormuz now supplies content-free evaluation tooling, not a representative customer corpus or a promotion decision.
...
The broader boundary remains governed by [accepted ADR 0004](decisions/0004-structured-dlp-and-approval-boundary.md). Source classification, application-specific query parsing beyond the bounded decoded views, semantic detection, unknown encoding/container coverage and archive-content inspection, actual organization-representative evaluation and threshold approval, approval notification/queue UX, production HA/backup/KMS proof, provider-native prompt-cache controls, and independent review are still open. PostgreSQL schema v6 retains the bounded multi-instance approval-persistence and atomic-consumption proof, but issue #10 remains open until the remaining paths and complete compatibility, failure, migration, and privacy gates pass.
...
python3 -m unittest -v \
  ...
  tests.test_gateway.GatewayIntegrationTests.test_nested_percent_encoded_query_is_inspected_for_both_providers \
  tests.test_gateway.GatewayIntegrationTests.test_nested_detect_only_query_is_forwarded_raw_once \
  ...
  tests.test_gateway.GatewayIntegrationTests.test_excessively_nested_percent_encoded_query_fails_content_free \
  tests.test_gateway.GatewayIntegrationTests.test_base64_tool_payload_secrets_are_redacted_for_both_providers \
  tests.test_redaction.SecretRedactorTests.test_percent_and_hex_encoded_sensitive_text_fail_closed_without_rewriting \
  tests.test_redaction.SecretRedactorTests.test_percent_and_hex_encoded_text_is_bounded_and_safe_values_are_preserved \
  tests.test_gateway.GatewayIntegrationTests.test_percent_and_hex_tool_payload_secrets_are_denied_for_both_providers \
