...
The broader boundary remains governed by [accepted ADR 0004](decisions/0004-structured-dlp-and-approval-boundary.md). Source classification, application-specific query parsing beyond the bounded decoded views, semantic detection, unknown encoding/container coverage and archive-content inspection, actual organization-representative evaluation and threshold approval, approval notification/queue UX, production HA/backup/KMS proof, content-cache invalidation, and independent review are still open. PostgreSQL schema v4 now proves bounded multi-instance approval persistence and atomic consumption, but issue #10 remains open until the remaining paths and complete compatibility, failure, migration, and privacy gates pass.
