...
Token volume and spend measure consumption, not employee productivity or work quality. Do not use this report as a performance ranking. `status` covers only requests that passed through Hormuz. Billing reconciliation separately labels the provider report's completeness as unverifiable from an offline response, so neither surface should be presented as complete organizational usage or employee performance evidence.
