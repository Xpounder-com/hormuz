...
- opt-in PostgreSQL schema version 2 for usage, cost evidence, usage-read audit, and atomic cross-replica budget reservations, with packaged checksummed migrations, separate owner/runtime roles, mandatory immutable tenant keys, forced RLS, transaction-local tenant context, and a digest-pinned PostgreSQL `16.14` two-tenant accounting proof; sessions, approvals, governed context, cutover backfill, pooling, backup/restore, KMS, HA, and production persistence remain unverified;
