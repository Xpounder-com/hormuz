...
conformance runner and a content-free evidence schema; the actual live
single-host Cephadm proof and any resulting certification are intentionally
separate in [#60](https://github.com/Xpounder-com/hormuz/issues/60). A passing
single-host run establishes RGW-level enforcement only, never host-root or
production-immutability protection.
