...
- disabled-by-default gateway-side user-priority injection of verified organization/team/actor packs into supported OpenAI Responses, direct-query OpenAI compaction, and Anthropic Messages/token-count requests, including exact administrator-granted repository/branch/trusted-revision selection carried by supported Codex and Claude Code configuration, with monotonic repository/classification/model/client caps, consumed-header and post-injection DLP, content-free usage lineage, required-mode denials, bounded installed-client plus protocol-shaped compatibility evidence, and one fixed content-free live OpenAI governed-context compaction observation;
...
- opt-in PostgreSQL schema version 4 for usage, cost evidence, usage-read audit, atomic cross-replica budget reservations, configuration-seeded identities/authorization versions, secret-free policy projection, human sessions, DLP approvals, and security evidence, with packaged checksummed migrations, separate owner/runtime roles, mandatory immutable tenant keys, forced RLS, transaction-local tenant context, keyed opaque tenant routing, and a digest-pinned PostgreSQL `16.14` two-tenant accounting/session/approval proof; governed context, SCIM, cutover backfill, coordinated policy rollout, pooling, backup/restore, KMS, HA, and production persistence remain unverified;
...
The foundation is not an enterprise release. The accepted PostgreSQL schema is
wired into the live usage/accounting, human-session, DLP-approval, and security
repositories, while governed context remains single-node SQLite and the local
context codec is plaintext. Real-IdP validation, SCIM, source-specific lifecycle collectors and
...
[ADR 0004](decisions/0004-structured-dlp-and-approval-boundary.md) was accepted by the product owner on 2026-08-15. The `hormuz-deterministic-v2` detector/action subset, monotonic team/person overlays, JSON string-key, bounded three-view provider-query decoding, allowlisted forwarded-header fail-closed enforcement, bounded base64/percent/hex encoded-text inspection, recognized compression/archive-container denial, provider-format-aware opaque-media denial, PostgreSQL-backed multi-instance replay-safe approval workflow, and content-free aggregate detector evaluator now exist. Actual organization-representative evaluation and threshold approval, source classification, application-specific parsing beyond the bounded query views, unknown encoding/container and archive-content inspection, semantic detection, approval notification/queue UX, production HA/KMS proof, cache invalidation, and the full #10 release evidence remain open.

[ADR 0006](decisions/0006-automatic-context-injection.md) was accepted by the product owner on 2026-08-16. It authorizes gateway-side, user-priority automatic context injection under #5 with injection disabled by default and verified-only as the production default. Exact repository grants, supported-client selectors, branch/revision narrowing, classification caps, direct-query OpenAI compaction, consumed-header/post-injection DLP, and content-free lineage are implemented, with one fixed live OpenAI compact observation; continuation binding remains a separate material owner decision, and acceptance does not close broader provider, quality, hosted persistence, or enterprise release gates.
