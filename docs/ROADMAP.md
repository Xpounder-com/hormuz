...
- local and GitHub package/client verification, including a six-wheel Python 3.11+ cross-platform hash-locked build closure, no backend re-resolution, two independent checked-out-commit package builds with byte-identical wheels and canonical source archives, plus two clean exact-source `linux/amd64` BuildKit exports with strict OCI validation and byte-equality evidence; this does not yet prove `linux/arm64` or universal cross-builder equality, offline build-input availability, hash-locked non-build runtime/test resolution, or an observed published release;
