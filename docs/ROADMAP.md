...
- opt-in PostgreSQL schema version 3 for usage, cost evidence, usage-read audit, atomic cross-replica budget reservations, configuration-seeded identities/authorization versions, and human sessions, with packaged checksummed migrations, separate owner/runtime roles, mandatory immutable tenant keys, forced RLS, transaction-local tenant context, keyed opaque tenant routing, and a digest-pinned PostgreSQL `16.14` two-tenant accounting/session proof; DLP approvals, governed context, SCIM, cutover backfill, pooling, backup/restore, KMS, HA, and production persistence remain unverified;
...
wired into the live usage/accounting and human-session repositories, while DLP
approvals and governed context remain single-node SQLite and the local context
codec is plaintext. Real-IdP validation, SCIM, source-specific lifecycle collectors and
