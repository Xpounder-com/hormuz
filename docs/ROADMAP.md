...
- shallow versioned liveness/readiness probes, atomic parsed-request capacity with saturation-aware readiness, a total provider-response relay deadline, and bounded `SIGTERM` request draining, with idle keep-alive, in-flight, continuous-trickle, and content-free failure regression coverage;
