...
| OpenAI Responses subset | Blocking `POST /v1/responses` and direct-query `POST /v1/responses/compact` tests through a protocol-shaped loopback fake, plus opt-in content-free live connectivity, synthetic-secret redaction, deprecated experimental context compaction, and pinned Codex `0.147.0` observations on macOS 26.2 arm64 | The blocking support level remains `protocol_tested`. Context-specific compaction evidence is retained only as historical compatibility evidence under ADR 0008, not as a supported product or release criterion. OpenAI exposes no hard output cap for compaction, so the configured allowance is reservation-only and actual provider usage can exceed it. The live observations are not complete DLP, provider SLA, quota, retention, residency, availability, every-model, or production-readiness proof. |
...
| Generic OIDC | Discovery/JWKS, mapped JWTs, authorization code plus PKCE, refresh rotation, replay revocation against a standards-shaped fake IdP, shared PostgreSQL session persistence, and a single-node SQLite SCIM lifecycle contract | No real vendor-specific IdP or SCIM server is certified. Shared directory persistence, production key custody, and HA remain open. |
...
| PostgreSQL | Opt-in schema-version-13 usage/cost/budget, desired-state identity and policy, immutable policy-version/active-pointer, provider-cache capability catalog, authorization-version, human-session, DLP-approval, security-event, identity-type usage, and shared-SCIM repositories; authenticated policy API/CLI; request-time exact-version enforcement; and a digest-pinned PostgreSQL `16.14` two-tenant observation including competing budget writers, session refreshers, approval retries, activation, rollback, and usage-version evidence | `development_only`: real deployment rollout, shared SCIM directory, cutover backfill, pooling, backup/PITR, restore, deletion, KMS, HA, and production-persistence proof remain open. The deprecated context experiment is deliberately excluded from the production persistence plan. |
...
The deprecated experimental context-compaction observation is recorded in
...
The accepted schema-v6 accounting, identity/session, policy/approval, and runtime-isolation observation is recorded
