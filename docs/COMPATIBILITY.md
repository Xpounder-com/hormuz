...
| PostgreSQL | Opt-in schema-version-7 usage/cost/budget, desired-state identity and policy, immutable policy-version/active-pointer, authorization-version, human-session, DLP-approval, security-event, and identity-type usage repositories; authenticated policy API/CLI; request-time exact-version enforcement; and a digest-pinned PostgreSQL `16.14` two-tenant observation including competing budget writers, session refreshers, approval retries, activation, rollback, and usage-version evidence | `development_only`: real deployment rollout, shared SCIM directory, cutover backfill, pooling, backup/PITR, restore, deletion, KMS, HA, and production-persistence proof remain open. The deprecated context experiment is deliberately excluded from the production persistence plan. |
