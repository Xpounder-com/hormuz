...
| PostgreSQL | Opt-in schema-version-5 usage/cost/budget, desired-state identity and policy, immutable policy-version/active-pointer, authorization-version, human-session, DLP-approval, and security-event repositories; packaged checksummed migrations and a digest-pinned PostgreSQL `16.14` two-tenant observation, including competing budget writers, session refreshers, approval retries, policy activation, and rollback | `development_only`: the policy administration API/CLI and request-time active-policy evaluation, SCIM, cutover backfill, pooling, backup/PITR, restore, deletion, KMS, HA, and production-persistence proof remain open. The deprecated context experiment is deliberately excluded from the production persistence plan. |
