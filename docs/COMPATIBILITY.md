...
| PostgreSQL | Opt-in schema-version-3 usage/cost/budget, desired-state identity, authorization-version, and human-session repositories; packaged checksummed migrations and a digest-pinned PostgreSQL `16.14` two-tenant observation, including competing budget writers and session refreshers | `development_only`: DLP approvals and governed context remain SQLite-backed; SCIM, usage backfill, pooling, backup/PITR, restore, deletion, KMS, HA, and production-persistence proof remain open. |
...
The accepted schema-v3 accounting, identity/session, and runtime-isolation observation is recorded
