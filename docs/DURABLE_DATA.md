...
| `portfolio_work_budget_accounting` | `portfolio_work_budget_reservation_bindings` | `portfolio_work_budget_reservation_bindings` | Immutable attempt-to-plan bindings with exact attribution, plan generation, configured route-rate identity, policy identity, model reference, reserved amount/output tokens and valuation-rule identity. Window and currency are foreign-key-bound to the exact plan version; request-time accounting fails closed at 10,000 bindings per plan window. These are conservative gateway estimates, not provider invoices or general-ledger reconciliation. |
