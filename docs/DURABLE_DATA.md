...
The native-attempt finance runtime adds `gateway_finance_attempt_evidence` in
SQLite migration 11 / PostgreSQL migration 15 and five begin-time rate-card
binding columns on request roots. It stores only a bounded provider metadata
...
The provider-free reconciliation successor adds separate snapshot receipts and
snapshot context events in SQLite migration 15 / PostgreSQL migration 20. It
accepts only signed, enrolled metadata pages, enforces one reconciliation ID
and page count per snapshot, shares context ordering with webhook evidence, and
emits no outcomes. See [LINEAR_RECONCILIATION.md](LINEAR_RECONCILIATION.md).
The run-to-outcome association successor adds five append-only metadata tables
in SQLite migration 16 / PostgreSQL migration 21. Explicit links bind exact
attempt, attribution, connector observation, object revision, use-case version,
and historical source-binding identities. Deterministic decisions preserve
unmatched, ambiguous, associated, and excluded states; the internal metric
reference deduplicates attempt cost and current work objects without persisting
a scorecard or claiming causality. See [ASSOCIATION_RUNTIME.md](ASSOCIATION_RUNTIME.md).
The model-scorecard successor adds immutable snapshots and append-only build
audit facts in SQLite migration 17 / PostgreSQL migration 22. It stores the
canonical metadata-only kernel input and evaluation so an exact result can be
recomputed and digest-checked; every result remains an association with bounded
