...
The workflow separates compilation from credential use. An unprivileged job tests and builds the universal executable, records its source commit, architectures, and SHA-256 digest, and transfers only that unsigned payload for one day. A fresh runner in the protected environment downloads it, repeats the commit, architecture, and digest checks, and only then enters the one step that receives the five secrets. Neither tests nor Swift compilation run on the credential-bearing runner.
