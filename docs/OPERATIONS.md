...
The connection ceiling is deliberately higher than the parsed-request default so ordinary clients and probes have header/keep-alive headroom. Both ceilings are hard process-local maxima, not workload-sizing recommendations. The header deadline ends when parsing completes and the body deadline begins at that boundary for body-bearing methods. These controls do not bound the kernel accept backlog, provide per-source rate limits, guarantee an operating-system DNS lookup deadline, coordinate capacity across replicas, or replace reverse-proxy/WAF TLS, connection, header, request-rate, and independently configured slow-body controls. Those remain required deployment work under issue #11.
