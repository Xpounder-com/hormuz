...
Health probes do not consume an application-request slot, so liveness and readiness remain observable at the configured capacity ceiling. These are shallow process probes. They do not query an upstream model provider, OIDC issuer, SQLite database, external KMS, or any future shared service. A provider or identity outage therefore does not turn liveness into a process restart loop. Dependency monitoring and a supported multi-node readiness policy remain open production-design work.
...
Hormuz applies three application-level limits:
...
These limits are process-local. They do not cap TCP connections or threads before a request is parsed, impose a complete ingress-header deadline, guarantee an operating-system DNS lookup deadline, coordinate capacity across replicas, or replace reverse-proxy/WAF connection, header, request-rate, and slow-client controls. Those remain required deployment work under issue #11.
