...
This contract does not make the alpha production-ready. The reference server is plain HTTP and is intended for loopback or a separately hardened private TLS boundary. Its usage, approval, context, and session stores are local SQLite implementations; there is no accepted shared persistence topology, HA/failover proof, backup/restore proof, RPO/RTO, container/reference deployment, deep dependency probe, shared load-balancer drain protocol, or independent security review. Those gates remain tracked in the enterprise roadmap and production-deployment issue.
