...
Hormuz applies five process-local limits:
...
The connection ceiling is deliberately higher than the parsed-request default so ordinary clients and probes have header/keep-alive headroom. Both ceilings are hard process-local maxima, not workload-sizing recommendations. The header deadline ends when parsing completes; `max_request_bytes` bounds body size but Hormuz does not yet impose a total ingress-body wall-clock deadline. These controls also do not bound the kernel accept backlog, provide per-source rate limits, guarantee an operating-system DNS lookup deadline, coordinate capacity across replicas, or replace reverse-proxy/WAF TLS, connection, header, request-rate, and slow-body controls. Those remain required deployment work under issue #11.
