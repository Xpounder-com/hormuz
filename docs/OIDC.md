...
Subject mapping uses the pair `(issuer, sub)`, not an email address or a caller-provided team claim. With the directory disabled, the configuration mapping is the authorization boundary that assigns the employee to an actor, team, organization, clearance, and allowed clients. With either SCIM adapter enabled, a live directory binding takes precedence: active memberships are resolved through tenant policy bindings keyed by stable SCIM group `externalId` on every request. In PostgreSQL mode, Hormuz resolves the tenant through an HMAC-tagged exact lookup before opening its tenant-bound RLS transaction; raw issuer/subject values remain in the tenant directory records, not the global lookup relation. If the same actor has both static and OIDC credentials, all configuration-seeded authorization fields must match. See [SCIM.md](SCIM.md) for the directory-specific rollout and lifecycle semantics.
...
The PostgreSQL `session_admin` path provides tenant-scoped multi-instance revocation and cursor-paginated, metadata-only logout, refresh-replay, mapping-removal, and administrative-revocation evidence. A digest-pinned local PostgreSQL gate proves cross-instance enrollment/authentication, competing refresh rotation, replay-family revocation, configuration-seeded authorization-version invalidation, shared SCIM CRUD, generic OIDC directory resolution, and directory-triggered session revocation. This query path is not an immutable audit sink. Real-IdP SCIM conformance, workload identity exchange, KMS custody/rotation, signed or externally immutable security-event export, retention controls, distributed enrollment throttling, pooling, HA, and backup/restore remain enterprise gates. `hormuz logout` provides employee-initiated revocation.
