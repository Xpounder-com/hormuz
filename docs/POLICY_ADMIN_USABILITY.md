...
receives only the publisher token and performs bounded read-only GitHub API
requests to authenticate its actor and repository access before the one
permitted build. It performs no mutation and emits no credential material.
...
step executes no checked-out code and independently repeats the credential and
live-control checks immediately before mutation. Revoke or rotate the publisher
token after a successful freeze.
