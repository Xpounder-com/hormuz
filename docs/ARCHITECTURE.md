...
- `hormuz/_persistence.py` owns backend-neutral durable-state values and normalized audit-verification inputs. `hormuz/store.py` retains SQLite SQL, snapshots, schema, transactions, and verification; `hormuz/postgres_usage_store.py` retains PostgreSQL SQL, locking, tenant scope/RLS, custody-source retrieval, transactions, and verification. In PostgreSQL gateway mode, `hormuz.postgres.PostgresConnectionPool` supplies one bounded runtime pool shared with managed-policy reads; each checkout receives fresh transaction-local role, search-path, and tenant state.
