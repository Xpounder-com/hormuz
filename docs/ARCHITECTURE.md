...
Custody authorization (managed PostgreSQL mode)
        ...
        +--> isolated executor claim + pending attempt before routine KMS work
...
- `hormuz/config.py` remains the public configuration-model façade. `hormuz/_config_builder.py` preserves the ordered construction and secret-resolution boundary, while `_config_input.py` strictly parses JSON and the `_config_custody.py`, `_config_identity.py`, `_config_ingress.py`, `_config_persistence.py`, `_config_policy.py`, and `_config_routing.py` modules own stable domain construction. Shared scalar validation lives in `_config_values.py`; the builder alone selects `os.environ`, then passes an explicit mapping to the final credential resolvers in identity, ingress, and policy order.
...
- `hormuz/custody_executor.py`, `hormuz/postgres_custody_executor_store.py`, `hormuz/custody_lifecycle.py`, and `hormuz/postgres_custody_lifecycle_store.py` are the separately credentialed governed-custody boundary. They claim one exact, current authorization before any side effect, retain only descriptor hashes plus append-only execution/lifecycle evidence, and derive monotonic runtime restrictions atomically from immutable events.
- `hormuz/_persistence.py` owns backend-neutral durable-state values and normalized audit-verification inputs. `hormuz/_audit_verifier.py` owns pure ordering, digest, epoch-linkage, checkpoint, truncation, source-event, and schema-version verification over one normalized snapshot. `hormuz/store.py` retains SQLite SQL, snapshot acquisition, schema, transactions, and SQLite error translation; `hormuz/postgres_usage_store.py` retains PostgreSQL SQL, locking, tenant scope/RLS, custody-source retrieval, transactions, and PostgreSQL error translation. In PostgreSQL gateway mode, `hormuz.postgres.PostgresConnectionPool` supplies one bounded runtime pool shared with managed-policy reads; each checkout receives fresh transaction-local role, search-path, and tenant state.
...
service/database role, the isolated machine executor, customer KMS authority, and
