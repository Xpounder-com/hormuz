...
- The server supplies a bounded accept-backlog hint at listening-socket activation and also owns a pre-thread accepted-connection ceiling, one server-wide absolute request-header deadline watchdog, a handler-enforced absolute request-body deadline with exact `Content-Length` ingestion, shallow content-free liveness/readiness, atomic parsed-request capacity, saturation responses, a total provider-response relay deadline, and bounded drain accounting. Health probes bypass application capacity but still require a connection slot; readiness reports parsed-request saturation. `SIGTERM` orchestration and its configured grace deadline live in `hormuz/cli.py`. These process-local controls do not provide per-source/distributed limits, operating-system SYN-queue policy, dependency-wide health, or HA.
...
- `hormuz/session_admin_client.py` owns the authenticated, redirect-refusing session-administration CLI transport and validates the metadata-only session and security-event response contracts. `hormuz/audit_admin_client.py` provides the same strict transport boundary for independent `audit_viewer` reads of tenant-scoped audit events.
...
- `hormuz/postgres.py` owns the accepted hosted schema migration ledger,
...
create schema objects or mutate the migration ledger. Every application
...
OIDC supports both a workload resource-server path and a human authorization-code + PKCE session path. Discovery and JWKS metadata are cached, an unknown signing-key ID triggers one refresh, and authorization attributes come only from a configured or live-directory `(issuer, subject)` mapping. Hormuz does not trust caller-provided group or team claims. Lifecycle connectors use this same resource-server path and additionally require server-configured `context_promoter`; the bearer token proves the mapped workload identity, not the truth of a claimed CI or source event. Usage administration independently requires one scoped usage-report capability: self, current-team aggregate, finance aggregate, or organization administrator. Audit administration separately requires `audit_viewer` and derives both the organization and immutable page window from the authenticated request. Report, self-usage, secret-summary, budget-total, active-reservation, and audit-event queries never accept a caller-selected tenant. Every session binds organization, actor, team, clearance, client, and database authorization version; desired-state synchronization or a capability-gated administrative action invalidates it before provider work. The local [SCIM lifecycle](SCIM.md) supplies a single-node dynamic mapping and eager session cleanup; PostgreSQL supplies immediate multi-instance session visibility. Real-IdP validation, shared directory persistence, SCIM vendor certification, KMS custody/rotation, pooling, backup/restore, and HA remain enterprise milestones. See [OIDC.md](OIDC.md), [SCIM.md](SCIM.md), [CONTEXT_LIFECYCLE_API.md](CONTEXT_LIFECYCLE_API.md), [SESSION_ADMIN_API.md](SESSION_ADMIN_API.md), [USAGE_ADMIN_API.md](USAGE_ADMIN_API.md), and [AUDIT_ADMIN_API.md](AUDIT_ADMIN_API.md).
