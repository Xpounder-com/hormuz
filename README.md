...
- Fail-closed configuration parsing that rejects unknown root and nested fields without reflecting attacker-chosen keys, validates model/fallback references, and refuses unknown or cross-organization-ambiguous team/person policy scopes.
...
- Exact-source `linux/amd64` OCI rebuilding exports one committed revision twice through a reviewed digest-pinned BuildKit, validates every referenced OCI descriptor and layer, requires byte-identical layouts, and emits a deterministic image tar plus content-free digest evidence before either artifact is accepted.
...
Configuration is immutable for a running process. Validate a candidate with `hormuz --config CANDIDATE doctor`, then use a readiness-gated replacement rollout and retain the previous image/configuration pair for rollback. See [docs/CONFIGURATION.md](docs/CONFIGURATION.md) for strict parsing, secret, change, and rollback semantics.
