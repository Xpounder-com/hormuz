...
The first executable milestone enforces client, model, output-token, monthly-token, team-budget, per-person budget, and per-model capacity rules. It records metadata-only usage in SQLite by default or in the opt-in PostgreSQL accounting and human-session store, estimates cost from configured rate cards, and keeps provider API keys on the Hormuz server rather than distributing them to employees.
...
- An opt-in PostgreSQL backend for usage, cost evidence, usage-read audit, cross-replica atomic budget reservations, configuration-seeded identity projection, and multi-instance human sessions. It uses checksummed migrations, separate owner/runtime roles, mandatory tenant keys, forced row-level security, transaction-local tenant binding, keyed opaque tenant routing tags, authorization-version invalidation, and a digest-pinned two-tenant integration gate. DLP approvals and governed context remain on their separately identified SQLite stores in this transition; see [docs/POSTGRESQL.md](docs/POSTGRESQL.md).
