...
- An opt-in, content-free SCIM identity lifecycle for users, groups, and federated workload identities. It supports a local SQLite adapter for one host and a tenant-isolated PostgreSQL adapter for shared deployments; both apply live generic OIDC identity, team/client/capability mapping, deactivation, and session invalidation. SCIM vendor certification, KMS custody, pooling/HA, and independent review remain open. See [docs/SCIM.md](docs/SCIM.md).
