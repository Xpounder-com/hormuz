...
- An accepted, versioned PostgreSQL schema foundation with checksummed migrations, separate owner/runtime roles, mandatory tenant keys, composite tenant foreign keys, forced row-level security, transaction-local tenant binding, and a digest-pinned two-tenant integration gate. The live gateway repositories remain SQLite-backed until their shared repository contracts and migration gates pass; see [docs/POSTGRESQL.md](docs/POSTGRESQL.md).
