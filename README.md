...
The first executable milestone enforces client, model, output-token, monthly-token, team-budget, per-person budget, and per-model capacity rules. It records metadata-only usage in SQLite by default or in the opt-in PostgreSQL accounting, session, and DLP/security store, estimates cost from configured rate cards, and keeps provider API keys on the Hormuz server rather than distributing them to employees.
...
- Monotonic organization/team/person control of known client-requested OpenAI and Anthropic prompt-cache directives, with no directive injection or rewrite; provider-automatic and unknown cache behavior remains explicitly observed-only. See [provider-native cache policy](docs/PROVIDER_CACHE_POLICY.md).
...
- Content-free control-plane observability: the selected SQLite or PostgreSQL accounting, human-session, DLP/security, and approval ledgers plus built-in HTTP access logs retain bounded metadata but not prompts, responses, raw request targets/queries/headers, OAuth codes, filenames, source content, matched values, credentials, or internal provider exceptions. A versioned exact-column manifest makes the SQLite telemetry stores fail closed at startup if an unreviewed field appears; the PostgreSQL schema is migration- and column-bound. Prompts and responses are inspected and relayed transiently, not persisted in those surfaces; the governed-context repository is a separate, intentionally content-bearing store.
...
- Independent `audit_viewer` authorization for tenant-scoped, metadata-only `hormuz audit events` reads: no caller-selected tenant, frozen-window cursor pagination, mandatory `security.admin.audit_read` evidence before response release, and a strict client that rejects cross-tenant or content-bearing response fields. The local `audit-export` command remains a separate operator-controlled export surface.
...
- An opt-in PostgreSQL backend for usage, cost evidence, administrative-read audit, cross-replica atomic budget reservations, configuration-seeded identity and secret-free policy projections, immutable live policy versions, multi-instance human sessions, one-time DLP approvals, and security evidence. It uses checksummed migrations, separate owner/runtime roles, mandatory tenant keys, forced row-level security, transaction-local tenant binding, keyed opaque tenant routing tags, authorization-version invalidation, and a digest-pinned two-tenant integration gate. Governed context remains on its separately identified SQLite store in this transition; see [docs/POSTGRESQL.md](docs/POSTGRESQL.md).
- An opt-in, content-free SCIM identity lifecycle for users, groups, and federated workload identities. It supports a local SQLite adapter for one host and a tenant-isolated PostgreSQL adapter for shared deployments; groups supply only lifecycle membership while tenant policy binds stable SCIM group external IDs to approved Hormuz authorization profiles. Deactivation and session invalidation are live; SCIM vendor certification, KMS custody, pooling/HA, workload-profile convergence, and independent review remain open. See [docs/SCIM.md](docs/SCIM.md).
