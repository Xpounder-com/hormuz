...
No OpenAI or Anthropic account is required. A successful run reports:
...
| [Signed OCI image](docs/OCI.md) | Immutable Linux `amd64` application artifact | Digest is the artifact contract; no mutable `latest` tag |
...
| Deferred | Broad external onboarding validation, general production HA/DR, cloud certification, complete provider-account coverage, and independent review. |
