...
- Pre-provider JSON string value/key and allowlisted provider-header inspection for credentials, valid hyphenated US SSNs, Luhn-valid cards, low-confidence email syntax, and organization dictionaries, with provider/model-scoped detect, redact, deny, or approval-required actions. Keys and forwarded header values are never mutated: enforced redaction findings in either fail closed. Identity-derived team/person DLP overlays can narrow scope and only strengthen organization actions. Bounded UTF-8 base64/base64url strings and textual data URIs are decoded, recursively inspected, and safely re-encoded after value transformation. Recognized OpenAI and Anthropic image/file content is denied by default when its bytes cannot be inspected; inspectable inline text documents continue through the ordinary redaction path. Optional approvals are metadata-only, non-self, exact-request-material/model bound, 15-minute, and atomically single-use.
