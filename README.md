...
The first executable milestone enforces client, model, output-token, monthly-token, team-budget, per-person budget, and per-model capacity rules. It records metadata-only usage in SQLite by default or in the opt-in PostgreSQL accounting store, estimates cost from configured rate cards, and keeps provider API keys on the Hormuz server rather than distributing them to employees.
...
- An opt-in PostgreSQL backend for usage, cost evidence, usage-read audit, and cross-replica atomic budget reservations, built on checksummed migrations, separate owner/runtime roles, mandatory tenant keys, forced row-level security, transaction-local tenant binding, and a digest-pinned two-tenant integration gate. Sessions, DLP approvals, and governed context remain on their separately identified SQLite stores in this transition; see [docs/POSTGRESQL.md](docs/POSTGRESQL.md).
